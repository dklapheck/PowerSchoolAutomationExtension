...
## Temporary settings capture (3.4.0)
...
5. Repeat for all four scenarios. Use `entryLogDate` in H for the currently captured form. Put `Attempt 1` in I only for SCC Attempt unless another workflow is separately verified in PowerSchool.

The recorder stores up to 20 selected-settings snapshots in extension storage; it does not store student numbers, note text, or arbitrary free-text fields. Version 3.4.0 applies the handoff date to the configured field and selects a configured tag by exact visible text. Review every prepared log before submitting.
