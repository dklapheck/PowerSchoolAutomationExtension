...
2. On **Instructions and Settings** in the roster, use the four rows **SCC Success**, **SCC Attempt**, **ECC Conversation**, and **ECC Attempt**. Enter the exact PowerSchool Log Type value in column B, Subtype value in D, log-date field ID or name in H, and exact visible tag label in I. Columns C and E are labels for reference; F can hold additional dropdown JSON. The extension reads the row sent with each handoff, so edits take effect on the next call without reinstalling. Leave a tag blank when that workflow does not need one. If an SCC Type/Subtype row is blank, select those two values manually on PowerSchool and click the blue SCC button.
3. The extension sets the requested log date, applies the configured tag, and inserts the call note while preserving PowerSchool's template. It stops on a missing, duplicated, or rejected configured control. Review the entire log and click **Submit** yourself. Older Apps Script handoffs without Settings values can still use previously remembered browser selections.
...
## Temporary settings capture (3.4.1)
...
4. With the SCC Attempt Type/Subtype selected, click **Copy Attempt Tag Map**. Paste that diagnostic into column J of another empty row. It records only visible Attempt 1–6 option labels and their control identifiers, so separate example logs are not required.
5. Click **Copy Type/Subtype for Settings** and paste into column **B** of that scenario's row in [Instructions and Settings](https://docs.google.com/spreadsheets/d/1_MpkySxTB6BYBB8In3ELRUsH2XpGjaeipMXxxGQb0To/edit#gid=1866668700): SCC Success row 35, SCC Attempt 36, ECC Conversation 37, ECC Attempt 38. This fills B:E. Column F optionally accepts valid extra-dropdown JSON such as `[{"name":"result","value":"no_answer"}]`; H contains the captured date control ID or name; I contains the exact visible tag option.
...
7. Repeat for all four scenarios. Use `entryLogDate` in H for the currently captured form. Put `Attempt 1` in I only for SCC Attempt unless another workflow is separately verified in PowerSchool.

The recorder stores up to 20 selected-settings snapshots in extension storage; it does not store student numbers, note text, or arbitrary free-text fields. Version 3.4.1 also provides a value-free date-field diagnostic; logging still applies the handoff date to the currently configured field and selects a configured tag by exact visible text. Review every prepared log before submitting.
