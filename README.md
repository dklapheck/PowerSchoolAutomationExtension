...
## Temporary settings capture (3.4.2)
...
The recorder stores up to 20 selected-settings snapshots in extension storage; it does not store student numbers, note text, or arbitrary free-text fields. Version 3.4.2 provides value-free date/tag diagnostics and safely matches a configured `Attempt 1` label to PowerSchool's displayed `Attempt 1 (34)` format. Review every prepared log before submitting.
